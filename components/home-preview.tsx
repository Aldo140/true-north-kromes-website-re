...
export function HomePreview() {
  ...
  return (
    <section className="bg-[#f2f2f2] py-20 lg:py-28" aria-label="Explore TNK">
      <div
        ref={ref}
          ...
          {sections.map((section) => {
            ...
            return (
              <Link
                key={section.href}
                href={section.href}
                className="group flex flex-col rounded-lg bg-white p-8 shadow-sm transition-shadow hover:shadow-md"
              >
                <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-full bg-[#f2f2f2] transition-colors group-hover:bg-[#1e6fff]/10">
                  <Icon className="h-5 w-5 text-[#1e1e1e] transition-colors group-hover:text-[#1e6fff]" />
                </div>
                <h3 className="text-base font-semibold uppercase tracking-[0.05em] text-[#1e1e1e]">
                  {section.title}
                </h3>
                <p className="mt-2 flex-1 text-sm leading-relaxed text-[#1e1e1e]/65">
                  {section.description}
                </p>
                <span className="mt-4 inline-flex items-center gap-1.5 text-sm font-medium text-[#1e6fff] transition-colors group-hover:text-[#1a5fd9]">
                  Learn more
